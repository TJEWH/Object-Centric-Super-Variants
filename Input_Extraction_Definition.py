...
class VariantLane:
    """The data structure of a lane of a variant corresponding to the activity execution of an object instance"""
    lane_id = 0
    object_type = ""
    lane_name = ""
    activities = []
    ...
    def to_super_lane(self, interactions):
        ...
        super_lane = SVD.SuperLane(self.lane_id,
                                   self.lane_name,
                                   self.object_type,
                                   elements,
                                   "1",
...
class RecursiveLanePosition(LanePosition):
    ...
    def add_level(self, new_position):
        """
        Adds another nested level to the position.
        :param self: The current position element
        :type self: RecursiveLanePosition
        :param new_position: The new base position element
        :type new_position: BasePosition
...
def extract_lanes(variant, frequency):
    """
    Extracts the variant from the corresponding variant layout and its frequency
    :param variant: The layout of a variant
    :type variant: dict
