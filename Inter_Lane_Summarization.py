...
def __inter_lane_summarization(lanes, interactions, print_results, current_lane=0, offset=0, intra=False, nested=False):
    ...
    :type intra: bool
    :param intra: Whether the to allow nested structures or not
    :type intra: bool
    ...
    if not nested:
        realization_lanes = [lane.get_valid_realizations() for lane in lanes]
    ...
    for common_element in longest_common_sequence:
            ...
            if current_positions[index] - last_positions[index] >= 0:
                ...
                for elem in all_lanes[j][1].elements:
                    if isinstance(elem, SVD.CommonConstruct):
                        if last_positions[index] <= elem.index < current_positions[index]:
        ...
        if nested:
            interval_elements, interval_length, interval_mapping = __apply_patterns_nested(interval_subprocesses,
                                                                                           current_horizontal_index,
                                                                                           interactions, base_lanes,
                                                                                           current_lane, print_results,
                                                                                           intra)
        else:
            interval_elements, interval_length, interval_mapping = __apply_patterns(interval_subprocesses,
                                                                                    current_horizontal_index,
                                                                                    interactions, base_lanes,
                                                                                    current_lane, print_results, intra)
        ...
        if is_interacting_activity:
            ...
            for i in range(len(base_lanes)):
                ...
                interaction_point_list = IED.get_interaction_points(interactions[i], base_lanes[i].lane_id,
                                                                    element.position)
            ...
            for i in range(len(interaction_points)):
                ...
                if ((identifier, interaction_points[i].activity_name,
                     ...
                     str(interaction_points[i].interaction_lanes)) in new_interaction_points_mapping.keys()):
                    new_interaction_points_mapping[(identifier, interaction_points[i].activity_name,
                                                    str([str(position) for position in
                                                         interaction_points[i].exact_positions]),
                                                    str(interaction_points[i].interaction_lanes))].append(
                        IED.BasePosition(current_lane, current_horizontal_index))
                else:
                    new_interaction_points_mapping[(identifier, interaction_points[i].activity_name,
                                                    str([str(position) for position in
                                                         interaction_points[i].exact_positions]),
                                                    str(interaction_points[i].interaction_lanes))] = [
                        IED.BasePosition(current_lane, current_horizontal_index)]

            elements.append(SVD.InteractionConstruct(common_element[0], element_frequency,
                                                     IED.BasePosition(current_lane, current_horizontal_index),
                                                     current_horizontal_index))

        else:
            elements.append(SVD.CommonConstruct(common_element[0], element_frequency,
                                                IED.BasePosition(current_lane, current_horizontal_index),
                                                current_horizontal_index))
    ...
    for j in range(len(all_lanes)):
        ...
        if current_position - last_positions[index] >= 0:
            ...
            for elem in all_lanes[j][1].elements:
                if isinstance(elem, SVD.CommonConstruct):
                    if last_positions[index] <= elem.index < current_position:
                        subprocess.append(elem)
                else:
    ...
    if nested:
        ...
        interval_elements, interval_length, interval_mapping = __apply_patterns(interval_subprocesses,
                                                                                current_horizontal_index, interactions,
                                                                                base_lanes, current_lane, print_results,
                                                                                intra)
...
def __apply_patterns_nested(interval_subprocesses, start_index, interactions, base_lanes, current_lane, print_results,
    ...
    choices = []
    total_frequency = sum([lane.frequency for lane in base_lanes])
    is_optional = False
    empty_frequency = 0
    current_choice = 0
    ...
    for elements in interval_subprocesses:
        ...
        for i in range(len(elements[1])):
            ...
            if (isinstance(elements[1][i], SVD.InteractionConstruct) or isinstance(elements[1][i],
                                                                                   SVD.GeneralChoiceStructure)):
                if isinstance(elements[1][i], SVD.InteractionConstruct):
                    interaction_points = IED.get_interaction_points(interactions[elements[0]],
                                                                    base_lanes[elements[0]].lane_id,
                    ...
                    for interaction_point in interaction_points:
                        ...
                        if ((identifier, interaction_point.activity_name,
                             ...
                             str(interaction_point.interaction_lanes)) in new_mapping.keys()):
                            new_mapping[(identifier, interaction_point.activity_name,
                                         str([str(position) for position in interaction_point.exact_positions]),
                                         str(interaction_point.interaction_lanes))].append(
                                IED.RecursiveLanePosition(current_lane, IED.BasePosition(current_choice, position)))
                        else:
                            new_mapping[(identifier, interaction_point.activity_name,
                                         str([str(position) for position in interaction_point.exact_positions]),
                                         str(interaction_point.interaction_lanes))] = [
                                IED.RecursiveLanePosition(current_lane, IED.BasePosition(current_choice, position))]
                    choice.append(SVD.InteractionConstruct(elements[1][i].activity,
                                                           base_lanes[elements[0]].frequency / total_frequency,
                                                           IED.RecursiveLanePosition(current_lane,
                                                                                     IED.BasePosition(current_choice,
                                                                                                      position)),
                                                           position))
                    length = 1
                else:
                    ...
                    for option in elements[1][i].choices:
                        ...
                        interaction_points = option.get_interaction_points(interactions[elements[0]],
                                                                           base_lanes[elements[0]].lane_id)
                        normalized_lane, positions_mapping = copy.deepcopy(option).normalize_option(current_lane,
                                                                                                    current_choice,
                                                                                                    position)
                            ...
                            for j in range(len(interaction_point.interaction_lanes)):
                                if interaction_point.interaction_lanes[j] == base_lanes[elements[0]].lane_id:
                                    ...
                                    if ((identifier, interaction_point.activity_name,
                                        ...
                                        new_mapping[(identifier, interaction_point.activity_name,
                                                     str([str(position) for position in
                                                          interaction_point.exact_positions]),
                                                     str(interaction_point.interaction_lanes))].append(new_position)
                                    else:
                                        new_mapping[(identifier, interaction_point.activity_name,
                                                     str([str(position) for position in
                                                          interaction_point.exact_positions]),
                                                     str(interaction_point.interaction_lanes))] = [new_position]
                                    break
                    ...
                    if isinstance(elements[1][i], SVD.OptionalConstruct):
                        choice.append(SVD.OptionalConstruct(new_choices, IED.RecursiveLanePosition(current_lane,
                                                                                                   IED.BasePosition(
                                                                                                       current_choice,
                                                                                                       position)),
                                                            IED.RecursiveLanePosition(current_lane,
                                                            ...
                                                            elements[1][i].empty_frequency))
                    else:
                        choice.append(SVD.ChoiceConstruct(new_choices, IED.RecursiveLanePosition(current_lane,
                                                                                                 IED.BasePosition(
                                                                                                     current_choice,
                                                                                                     position)),
                                                          IED.RecursiveLanePosition(current_lane,
                ...
                choice.append(
                    SVD.CommonConstruct(elements[1][i].activity, base_lanes[elements[0]].frequency / total_frequency,
                                        IED.RecursiveLanePosition(current_lane,
                                                                  IED.BasePosition(current_choice, position)),
                                        position))
                length = 1
        ...
        if not choice:
            ...
            for j in range(len(choices)):
                is_identical, update_lane = choices[j].check_identical(choice)

                if is_identical:
                    choices[j] = update_lane
                    choices[j].cardinality = "n"
                    choices[j].frequency += base_lanes[elements[0]].frequency
                    duplicate = True
                    break

            if not duplicate:
                choices.append(
                    SVD.SuperLane(current_choice, "Option " + str(current_choice), base_lanes[0].object_type, choice,
                                  "1", base_lanes[elements[0]].frequency, []))
                current_choice += 1

        end_index = max(end_index, position - 1)

    if (len(choices) == 1 and len(choices[0].elements) == 1 and isinstance(choices[0].elements[0],
                                                                           SVD.GeneralChoiceStructure)):
        ...
        for extracted_choice in choices[0].elements[0].choices:
            extracted_elements, extracted_positions_mapping = copy.deepcopy(extracted_choice).extract_option(
                extracted_choice.frequency / total_frequency)
            extracted_choices.append(
                SVD.SuperLane(_id, "Option " + str(_id), base_lanes[0].object_type, extracted_elements.elements, "1",
                              extracted_choice.frequency, []))
            _id += 1
        ...
        if isinstance(choices[0].elements[0], SVD.OptionalConstruct):
            empty_frequency = 1 - (
                    (empty_frequency / len(interval_subprocesses)) * choices[0].elements[0].empty_frequency)
        else:
            empty_frequency = empty_frequency / len(interval_subprocesses)

        returned_elements.append(SVD.OptionalConstruct(extracted_choices, IED.BasePosition(current_lane, start_index),
                                                       IED.BasePosition(current_lane, end_index), start_index,
                                                       end_index, empty_frequency))
    ...
    if (is_optional):
        ...
        if print_results:
            ...
            for choice in choices:
                ...
                print("----------")
        empty_frequency = empty_frequency / len(interval_subprocesses)
        returned_elements.append(SVD.OptionalConstruct(choices, IED.BasePosition(current_lane, start_index),
                                                       IED.BasePosition(current_lane, end_index), start_index,
                                                       end_index, empty_frequency))
    ...
    #Applying Exclusive Choice Pattern Pattern
        ...
        returned_elements.append(SVD.ChoiceConstruct(choices, IED.BasePosition(current_lane, start_index),
                                                     IED.BasePosition(current_lane, end_index), start_index, end_index))
...
def __apply_patterns(interval_subprocesses, start_index, interactions, base_lanes, current_lane, print_results,
        ...
        for i in range(len(elements[1])):
            ...
            if isinstance(elements[1][i], SVD.InteractionConstruct):
                ...
                for interaction_point in interaction_points:
                    ...
                    current_mappings[(identifier, interaction_point.activity_name,
                                      str([str(position) for position in interaction_point.exact_positions]),
                                      str(interaction_point.interaction_lanes))] = start_index + i
                    contains_interaction = True
                choice.append(SVD.InteractionConstruct(elements[1][i].activity, elements[1][i].frequency,
                                                       IED.BasePosition(current_lane, position), position))
            else:
                choice.append(SVD.CommonConstruct(elements[1][i].activity, elements[1][i].frequency,
                                                  IED.BasePosition(current_lane, position), position))
        ...
        if not choice:
            ...
            for i in range(len(choices)):
                ...
                if len(choices[i][0]) == len(choice):
                    for j in range(len(choices[i][0])):
                        equal = equal and (type(choices[i][0][j]) is type(choice[j]) and (
                                choices[i][0][j].activity == choice[j].activity))
            ...
            if not duplicate:
                ...
                for elem in choice:
                    ...
                    elem.position = IED.RecursiveLanePosition(current_lane,
                                                              IED.BasePosition(current_index, horizontal_position))
                    end_index = max(end_index, horizontal_position)
                choices.append(tuple((choice, "1", 1, {elements[0]})))
                for key in current_mappings.keys():
                    if not key in new_mapping:
                        new_mapping[key] = [IED.RecursiveLanePosition(current_lane, IED.BasePosition(current_index,
                                                                                                     current_mappings[
                                                                                                         key]))]
                    else:
                        new_mapping[key].append(IED.RecursiveLanePosition(current_lane, IED.BasePosition(current_index,
                                                                                                         current_mappings[
                                                                                                             key])))
    ...
    for i in range(len(choices)):
        ...
        choices[i] = SVD.SuperLane(i, "Option " + str(i), base_lanes[0].object_type, choices[i][0], choices[i][1],
                                   choices[i][2], [])
    ...
    # Applying Exclusive Choice Pattern
    else:
...
def __get_longest_common_subsequence(lanes, intra=False, interactions=None):
    ...
    :type lanes: List of type SuperLane
    :param intra: Whether the input lanes are from the same variant. Used to check for shared interacion points.
    :type intra: bool
    ...
    if any([lane[2] == 0 for lane in lanes]):
        ...
        intra_valid = False
        if (all((type(lane[1].elements[lane[2] - 1]) is type(base_element) and
                 lane[1].elements[lane[2] - 1].activity == base_element.activity) for lane in lanes)):
            intra_valid = True

            if intra and type(base_element) is SVD.InteractionConstruct:
                is_interacting, interaction_point = IED.is_interaction_point(interactions, lanes[0][1].lane_id,
        ...
        if intra_valid:
            recursive_result, recursive_value, recursive_interactions = __get_longest_common_subsequence(
                [(lane[0], lane[1], lane[2] - 1) for lane in lanes], intra, interactions)
            ...
            if type(base_element) is SVD.InteractionConstruct:
                recursive_interactions = recursive_interactions + 1
            return recursive_result + [
                (base_element.activity, indices, positions)], recursive_value + 1, recursive_interactions
            ...
            for i in range(1, pow(2, len(lanes)) - 1):
                ...
                recursive_result, recursive_value, recursive_interactions = __get_longest_common_subsequence(
                    recursive_call_lanes, intra, interactions)

                if ((recursive_value > maximum_value) or (
                        recursive_value == maximum_value and recursive_interactions > recursive_interactions)):
