...
def __inter_lane_summarization(lanes, interactions, current_lane=0, offset=0, intra=False, nested=False):
    ...
    if intra:
        longest_common_sequence, length, number_of_interactions = __get_longest_common_subsequence(
            base_lanes_indexed, intra, interactions[0])
    else:
        longest_common_sequence, length, number_of_interactions = __get_longest_common_subsequence(
            base_lanes_indexed)
    ...
    for common_element in longest_common_sequence:
        ...
        if nested:
            interval_elements, interval_length, interval_mapping = __apply_patterns_nested(
                interval_subprocesses,
        ...
        if is_interacting_activity:
            ...
            for i in range(len(base_lanes)):
                ...
                interaction_point_list = IED.get_interaction_points(
                    interactions[i],
                    base_lanes[i].lane_id,
                    element.position)
            ...
            for i in range(len(interaction_points)):
                ...
                if ((identifier, interaction_points[i].activity_name,
                    ...
                    (new_interaction_points_mapping[(
                        identifier,
                        interaction_points[i].activity_name,
                        str([str(position) for position in interaction_points[i].exact_positions]),
                        str(interaction_points[i].interaction_lanes))
                     ].append(IED.BasePosition(current_lane, current_horizontal_index)))
                else:
                    new_interaction_points_mapping[(
                        identifier,
                        interaction_points[i].activity_name,
        ...
        for i in range(len(elements[1])):
            if (isinstance(elements[1][i], SVD.InteractionConstruct) or
                ...
                if isinstance(elements[1][i], SVD.InteractionConstruct):
                    interaction_points = IED.get_interaction_points(
                    ...
                    for option in elements[1][i].choices:
                        interaction_points = option.get_interaction_points(
                            interactions[elements[0]], base_lanes[elements[0]].lane_id)
                        normalized_lane, positions_mapping = (
                            copy.deepcopy(option).normalize_option(current_lane, current_choice, position))
                        ...
                        for interaction_point in interaction_points:
                            for j in range(len(interaction_point.interaction_lanes)):
                                if interaction_point.interaction_lanes[j] == base_lanes[elements[0]].lane_id:
                                    ...
                                    if ((identifier,
                                         interaction_point.activity_name,
                                         str([str(position) for position in interaction_point.exact_positions]),
                                         str(interaction_point.interaction_lanes)) in new_mapping.keys()):
                                        new_mapping[(identifier,
                                                     interaction_point.activity_name,
                                                     str([str(position) for position in interaction_point.exact_positions]),
                    ...
                    if isinstance(elements[1][i], SVD.OptionalConstruct):
                        choice.append(
                            SVD.OptionalConstruct(
                                new_choices, IED.RecursiveLanePosition(
                                    current_lane,
                                    IED.BasePosition(current_choice, position)),
                                IED.RecursiveLanePosition(
                                    current_lane,
                                    IED.BasePosition(current_choice, position + length - 1)),
        ...
        if not choice:
            ...
            if not duplicate:
                choices.append(
                    SVD.SuperLane(
                        current_choice,
                        "Option " + str(current_choice),
                        base_lanes[0].object_type,
                        choice,
                        "1",
                        base_lanes[elements[0]].frequency,
                        []))
                current_choice += 1

        end_index = max(end_index, position - 1)

    if (len(choices) == 1 and len(choices[0].elements) == 1 and
            isinstance(choices[0].elements[0], SVD.GeneralChoiceStructure)):
        ...
        if isinstance(choices[0].elements[0], SVD.OptionalConstruct):
            empty_frequency = 1 - ((empty_frequency / len(interval_subprocesses)) *
                                   choices[0].elements[0].empty_frequency)
        else:
        ...
        returned_elements.append(
            SVD.OptionalConstruct(
                extracted_choices,
                IED.BasePosition(current_lane, start_index),
                IED.BasePosition(current_lane, end_index),
                start_index,
                end_index,
                empty_frequency))
        ...
        for i in range(len(elements[1])):
            ...
            if isinstance(elements[1][i], SVD.InteractionConstruct):
                ...
                choice.append(
                    SVD.InteractionConstruct(
                        elements[1][i].activity,
                        elements[1][i].frequency,
                        IED.BasePosition(current_lane, position),
                        position))
            else:
                choice.append(
                    SVD.CommonConstruct(
                        elements[1][i].activity, elements[1][i].frequency,
                        IED.BasePosition(current_lane, position),
                        position))
        ...
        if not choice:
            ...
            if not duplicate:
                ...
                for elem in choice:
                    ...
                    elem.position = IED.RecursiveLanePosition(
                        current_lane,
                ...
                for key in current_mappings.keys():
                    if key not in new_mapping:
                        new_mapping[key] = [IED.RecursiveLanePosition(
                            current_lane,
                            IED.BasePosition(current_index, current_mappings[key]))]
                    else:
                        new_mapping[key].append(
                            IED.RecursiveLanePosition(
                                current_lane,
                                IED.BasePosition(current_index, current_mappings[key])))
...
def __get_longest_common_subsequence(lanes, intra=False, interactions=None):
    ...
    :type lanes: List of type SuperLane
    :param intra: Whether the input lanes are from the same variant. Used to check for shared interaction points.
    :type intra: bool
    :param interactions: The interaction points of the variant for Intra-Variant Summarization setting
    ...
    if any([lane[2] == 0 for lane in lanes]):
        ...
        if intra_valid:
            ...
            return (recursive_result + [(base_element.activity, indices, positions)],
                    recursive_value + 1,
                    recursive_interactions)
            ...
            for i in range(1, pow(2, len(lanes)) - 1):
                ...
                if ((recursive_value > maximum_value) or (
                        recursive_value == maximum_value and
                        recursive_interactions > recursive_interactions)):
