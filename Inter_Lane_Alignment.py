...
def __split_interaction_mappings(mappings):
    ...
    for interaction in mappings.keys():
        positions = []
        for _id in mappings[interaction].keys():
            id_positions = []
            for position in mappings[interaction][_id]:
                id_positions.append((_id, position))
...
def __combine_interactions(mappings):
    ...
    for interaction in mappings.keys():
        ...
        if len(share_elements) == 1:
            ...
            if interaction not in added_keys:
                positions = dict()
...
def __re_align_lanes(lanes, mappings, intra=True, repeat=REPEAT_ALIGNMENT):
    ...
    for i in range(len(mappings.keys())):
        ...
        earliest_interaction_point = min(updated_mappings.items(),
                                         key=lambda x: tuple([_position.get_base_index() for _position in x[1].values()]))
        ...
        try:
            element = relevant_lanes[0].get_element(
                updated_mappings[earliest_interaction_point[0]][relevant_lanes[0].lane_id])
            activity_label = element.activity
        except:
            activity_label = "NaN"
                ...
                if offset == 0 or (not ALIGN and not intra):
                    ...
                    for key in updated_mappings.keys():
                        if lane.lane_id in updated_mappings[key].keys():
                            updated_positions[str(updated_mappings[key][lane.lane_id])] = updated_mappings[key][
                                lane.lane_id]
                    ...
                    if shift_allowed:
                        ...
                        for key in updated_mappings.keys():
                            if new_lane.lane_id in updated_mappings[key].keys():
                                updated_mappings[key][new_lane.lane_id] = copy.deepcopy(
                                    updated_positions[str(updated_mappings[key][new_lane.lane_id])])
                        ...
                        for interaction_point in updated_interaction_points:
                            for i in range(len(interaction_point.interaction_lanes)):
                                if i in range(len(interaction_point.exact_positions)):
                                    if (interaction_point.interaction_lanes[i] == lane.lane_id and str(
                                            interaction_point.exact_positions[i]) in updated_positions.keys()):
                                        interaction_point.exact_positions[i] = updated_positions[
                                            str(interaction_point.exact_positions[i])]
...
def create_duplicate_interaction(mappings, lanes):
    ...
    # For each set of interaction points sharing the same positions in one lane, but not in another lane,
    # split the element at the shared position such that every interaction point can be aligned with one copy
    for duplicate in duplicates:
        ...
        for i in range(len(duplicate[0])):
            ...
            current_interaction_point = min(duplicate[0].items(), key=lambda x: min(
                [position.get_base_index() for position in x[1].values()]))
                    ...
                    if (duplicate[1] in mappings[key].keys() and new_lane.greater_than(suitable_position,
                        ...
                        for lane_id in mappings[key].keys():
                            if lane_id != duplicate[1] and lane_id in watchlist:
                                ...
                                changes_position = current_lane.greater_than(earliest_interaction_point[1][lane_id],
                                                                             mappings[key][lane_id])
