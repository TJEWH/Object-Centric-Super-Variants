...
def arrange_super_variant(super_variant, ax, vertical_start_position, horizontal_start_position, suppression_char, mode,
                          fontsize_title, fontsize_activities, cut_off_point):
    """
    ...
    for i in range(len(super_variant.lanes)):
        ...
        ax.text(-20 + horizontal_start_position,
                current_vertical_position * DEFAULT_CHEVRON_HEIGHT + 
                0.5 * lane_properties[lane_id]["Height"] * DEFAULT_CHEVRON_HEIGHT - 0.3, 
                cardinality, zorder=10, fontsize=fontsize_title)
        ...
        ax.text(-14 + horizontal_start_position,
                current_vertical_position * DEFAULT_CHEVRON_HEIGHT + 
                0.5 * lane_properties[lane_id]["Height"] * DEFAULT_CHEVRON_HEIGHT - 0.3, 
                label, zorder=10, fontsize=fontsize_title)

        if lane_properties[lane_id]["IsOptional"]:
            ax.add_patch(
                patches.Rectangle(
                    (-15 + horizontal_start_position, current_vertical_position * DEFAULT_CHEVRON_HEIGHT),
                    (maximal_lane_length + 2) * DEFAULT_CHEVRON_LENGTH,
                    lane_properties[lane_id]["Height"] * DEFAULT_CHEVRON_HEIGHT, 
                    color=color, alpha=0.8, zorder=0, hatch='///'))
        else:
            ax.add_patch(
                patches.Rectangle(
                    (-15 + horizontal_start_position, current_vertical_position * DEFAULT_CHEVRON_HEIGHT),
        ...
        ax = __visualize_lane_elements(
            ax, lane_id, 
            super_variant.lanes[i].elements, 
            properties,
            copy.deepcopy(super_variant.interaction_points), 
    ...
    ax.text(-15 + horizontal_start_position,
            current_vertical_position * DEFAULT_CHEVRON_HEIGHT + 0.7 * 9 / fontsize_title,
            "Frequency: " + str(round(super_variant.frequency, 3)), 
            zorder=10, fontsize=fontsize_title)
    ax.text(-15 + horizontal_start_position,
            current_vertical_position * DEFAULT_CHEVRON_HEIGHT + 5 * 9 / fontsize_title,
            "Super Variant: " + str(super_variant.id), 
            zorder=10, fontsize=fontsize_title * 1.2, fontweight='bold')
...
def __visualize_lane_elements(ax, lane, elements, lane_properties, interaction_points, current_vertical_position,
                              current_horizontal_position, mode, fontsize, cut_off_point,
                              chevron_length=DEFAULT_CHEVRON_LENGTH, chevron_height=DEFAULT_CHEVRON_HEIGHT, offset=0,
                              frequency=False, original_lane=None, original_lane_properties=None):
    """
    ...
    :type lane: int
    :param elements: The elements that should be visualized
    :type elements: list of type SummarizationElement
    :param lane_properties: Relevant "Height", "Color" and "IsOptional" information for each lane in the Super Variant
    ...
    :param chevron_height: The height of one unit of a chevron
    :type chevron_height: float
    :param offset: The offset subtracted from the element indices
    :type offset: int
    ...
    if original_lane is None:
        original_lane = lane
    if original_lane_properties is None:
        original_lane_properties = lane_properties

    for element in elements:
        properties = copy.deepcopy(lane_properties)
        if isinstance(element, SVD.InteractionConstruct):
            ax = __interaction_activity_chevron(
                ax, lane, element, element.index - offset, properties, interaction_points, 
                current_vertical_position, current_horizontal_position, 
                chevron_length, chevron_height, frequency, fontsize, 
                original_lane, original_lane_properties, mode, cut_off_point)
        elif isinstance(element, SVD.CommonConstruct):
            ax = __common_activity_chevron(
                ax, lane, element, element.index - offset, properties,
                current_vertical_position, current_horizontal_position, 
                chevron_length, chevron_height, frequency, fontsize, 
                mode, cut_off_point)
        else:
            ax = __choice_structure_chevron(
                ax, lane, element, element.index_start - offset, properties, interaction_points, 
                current_vertical_position, current_horizontal_position,
                chevron_length, chevron_height, fontsize, 
                original_lane, original_lane_properties, mode, cut_off_point)
    return ax


def __interaction_activity_chevron(ax, lane, element, index, lane_properties, interaction_points,
                                   current_vertical_position, current_horizontal_position, chevron_length,
                                   chevron_height, frequency, fontsize, original_lane, original_lane_properties, mode,
                                   cut_off_point, overall_line_style='-'):
    """
    ...
    outline_color = "black"
    is_interacting, interaction_point = IED.is_interaction_point(
        interaction_points, original_lane, element.position)

    if not is_interacting:
        interacting_lanes = [[original_lane]]
        # TODO increase information gain
        logging.debug("Interaction not found.")
        if MARK_INCORRECT_INTERACTIONS:
            outline_color = "red"

    else:
        all_interaction_points = IED.get_interaction_points(
            interaction_points, original_lane, element.position)
        
        if len(all_interaction_points) == 1:
            interacting_lanes = [interaction_point.interaction_lanes]
            if (len(set([position.get_base_index() for position in
                         interaction_point.exact_positions])) > 1 and MARK_INCORRECT_INTERACTIONS):
                outline_color = "red"
        else:
            ...
            for i in range(1, len(all_interaction_points)):
                ...
                if (len(set([position.get_base_index() for position in
                             all_interaction_points[i].exact_positions])) > 1 and MARK_INCORRECT_INTERACTIONS):
                    outline_color = "red"
    ...
    ax.text(index * chevron_length + length_offset + current_horizontal_position,
            current_vertical_position * chevron_height + 0.5 * chevron_height * lane_properties[lane][
                "Height"] - height_offset, label, zorder=15, fontsize=fontsize * sizing_factor)
    ...
    for interacting_lanes_list in interacting_lanes:
        ...
        for i in range(len(interacting_lanes_list)):
            ...
            ax.add_patch(
                patches.PathPatch(
                    __partial_chevron_at_position(
                        current_horizontal_position + index * chevron_length + i * length_sub_chevron * chevron_length,
                        current_vertical_position * chevron_height,
                        length_sub_chevron * chevron_length / DEFAULT_CHEVRON_LENGTH,
                        lane_properties[lane]["Height"] * chevron_height, 
                        current_percentage, current_percentage + sub_height),
                    facecolor=color, 
                    lw=line_width, 
                    ls=overall_line_style, 
                    zorder=10))
    ...
    ax.add_patch(
        patches.PathPatch(
            __chevron_at_position(
                current_horizontal_position + index * chevron_length,
                current_vertical_position * chevron_height,
                chevron_length / DEFAULT_CHEVRON_LENGTH,
                lane_properties[lane]["Height"] * chevron_height),
            facecolor="None", 
            lw=1.3 * fontsize / 9, 
            ls=overall_line_style, 
...
def __common_activity_chevron(ax, lane, element, index, lane_properties, current_vertical_position,
                              current_horizontal_position, chevron_length, chevron_height, frequency, fontsize, mode,
                              cut_off_point, overall_line_style='-'):
    """
    ...
    ax.text(
        current_horizontal_position + index * chevron_length + length_offset,
        current_vertical_position * chevron_height + 
        0.5 * chevron_height * lane_properties[lane]["Height"] - height_offset, label, 
        zorder=15, fontsize=fontsize * sizing_factor)
    ...
    ax.add_patch(
        patches.PathPatch(
            __chevron_at_position(
                ...
                chevron_length / DEFAULT_CHEVRON_LENGTH,
                lane_properties[lane]["Height"] * chevron_height),
            facecolor=lane_properties[lane]["Color"], 
            lw=1.3 * fontsize / 9,
            ls=overall_line_style,
            zorder=10))
...
def __choice_structure_chevron(ax, lane, element, index, lane_properties, interaction_points, current_vertical_position,
                               current_horizontal_position, chevron_length, chevron_height, fontsize, original_lane,
                               original_lane_properties, mode, cut_off_point, overall_line_style='-'):
    """
    ...
    ax.add_patch(
        patches.PathPatch(
            __chevron_at_position(
                current_horizontal_position + index * chevron_length,
                current_vertical_position * chevron_height,
                (element.index_end - element.index_start + 1) * chevron_length / DEFAULT_CHEVRON_LENGTH,
                lane_properties[lane]["Height"] * chevron_height),
            facecolor="None", 
            lw=1 * fontsize / 9, 
            ls=overall_line_style, 
    ...
    sub_default_chevron_length = ((((element.index_end - element.index_start + 1) * chevron_length) - 2) / 
                                  ((element.index_end - element.index_start) + 1))
    ...
    for i in range(len(element.choices)):
        ...
        if vertical_position != current_vertical_position * chevron_height:
            ...
            ax.add_patch(
                patches.PathPatch(
                    __line_at_position(
                        current_horizontal_position + index * chevron_length + line_offset,
                        vertical_position, 
                        (element.index_end - element.index_start + 1) * chevron_length / DEFAULT_CHEVRON_LENGTH),
                    lw=1.1 * fontsize / 9, 
                    ls=overall_line_style, 
        ...
        ax = __visualize_lane_elements(
            ax, 
            choice,
            element.choices[i].elements, 
            copy.deepcopy(choice_properties),
            interaction_points, 
            ((vertical_position + margin_height * choice_properties[choice]["Height"]) / sub_default_chevron_height),
            horizontal_start_position, 
            mode, fontsize * (sub_default_chevron_length / DEFAULT_CHEVRON_LENGTH), 
            cut_off_point,
            chevron_length=sub_default_chevron_length,
            chevron_height=sub_default_chevron_height, 
            offset=offset + index, 
            frequency=True,
            original_lane=original_lane, 
            original_lane_properties=original_lane_properties)
        ...
        if mode == Mode.LANE_FREQUENCY:
            ...
            ax.plot(
                horizontal_start_position + 0.8 * chevron_length / DEFAULT_CHEVRON_LENGTH,
                vertical_position + choice_properties[choice]["Height"] * chevron_height * height_factor - 
                0.8 * chevron_length / DEFAULT_CHEVRON_LENGTH,
                'kv', zorder=25, markersize=8 * chevron_length / DEFAULT_CHEVRON_LENGTH)

            current_annotations.append((
                (horizontal_start_position + 0.8 * chevron_length / DEFAULT_CHEVRON_LENGTH,
                 vertical_position + choice_properties[choice]["Height"] * chevron_height * height_factor - 
                 1 * chevron_length / DEFAULT_CHEVRON_LENGTH),
                frequency, 8 * chevron_length / DEFAULT_CHEVRON_LENGTH))
...
def __chevron_at_position(horizontal_index, vertical_index, length, height):
    ...
    verts = [
        ...
        (horizontal_index + DEFAULT_CHEVRON_LENGTH * length + 1.25, vertical_index + height / 2),
        # Outer tip of chevron
...
def __partial_chevron_at_position(horizontal_index, vertical_index, length, height, start_percentage, end_percentage):
    ...
    if start_percentage >= 0.5:
        verts = [
            (horizontal_index + 1.25 - (start_percentage - 0.5) * 2.5, vertical_index + start_percentage * height),
            # Left bottom corner
            (horizontal_index + 1.25 - (start_percentage - 0.5) * 2.5 + DEFAULT_CHEVRON_LENGTH * length,
             vertical_index + start_percentage * height),  # Right bottom corner
            (horizontal_index + 1.25 - (end_percentage - 0.5) * 2.5 + DEFAULT_CHEVRON_LENGTH * length,
             vertical_index + end_percentage * height),  # Right upper corner
            (horizontal_index + 1.25 - (end_percentage - 0.5) * 2.5, vertical_index + end_percentage * height),
            # Left upper corner
            (horizontal_index + 1.25 - (start_percentage - 0.5) * 2.5, vertical_index + start_percentage * height),
            # Left bottom corner
        ]
        ...
        verts = [
            (horizontal_index + start_percentage * 2.5, vertical_index + start_percentage * height),
            # Left bottom corner
            (horizontal_index + start_percentage * 2.5 + DEFAULT_CHEVRON_LENGTH * length,
             vertical_index + start_percentage * height),  # Right bottom corner
            (horizontal_index + end_percentage * 2.5 + DEFAULT_CHEVRON_LENGTH * length,
             vertical_index + end_percentage * height),  # Right upper corner
